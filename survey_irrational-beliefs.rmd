...
# I though I would go for 3 factors  
...
anova(mod_51_graded2, mod_49_graded2, verbose = T) # p value is not significant, however, almost everything has imrpoved 

anova(mod_49_graded2, mod_graded2, verbose = T) # p value is not significant, however, almost everything has imrpoved 

stat_49_graded2 <- M2(mod_49_graded2) # didn't use type C2 since I have too many dfs 
 
ifit_ques49_graded <- mirt::itemfit(mod_49_graded2)

ifit_ques49_graded[ifit_ques49_graded[, 5] < 0.05, ] ## misfitting items

Mod2.REL <- fscores(mod_49_graded2, method= "EAP", returnER = T)


```


# try the model with gpcm 

mod_49_graded3gpcm <- mirt(ques49, 3, itemtype = "gpcm", TOL = 0.001)
...
foo <- summary(mod_49_graded3, rotate = "varimax")

fo2 <- data.frame(foo$rotF)
fo2[abs(fo2$F3) > abs(fo2$F2) & abs(fo2$F3) > abs(fo2$F1),]



foo <- summary(mod_49_graded2, rotate = "varimax")
